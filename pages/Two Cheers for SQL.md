...
I've worked with teams that have gone for years putting up with the inflexibility of their design because it has foreign keys constraints that enforce a particular order to the insertion of data, and they believed (through a kind of psychological conditioning) that if they removed the constraints then something bad would happen. In reality, nothing bad would happen and their design would start behaving in a way that would be immediately better for their users.
