...
[Redux](http://redux.js.org/) is based on series of really simple *what-if* questions:
...
But just how easy is it to make each bit of your application capable of finding and update the bit of data relevant to itself within *the entire state*? What about modularity, composability, loose coupling, minimal dependency? It seems that, when I want to change the title of a book, I need to know that the book is in a shelf and the shelf is in a shop, so I can create an the single global store how to update. The "leaf nodes" of my app seemingly need to know far too much about where they sit in the world. Famously, in purist functional programming, every time I change something I've *created a new version of the universe*.
